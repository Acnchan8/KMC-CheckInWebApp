...
def main(): 
    ...
    #file_path = "myproject\media\KMC_Student_Database.xlsx"
    #df = pd.read_excel(file_path)
    
    #check_in_df = pd.read_excel("myproject\media\KMC_Master_Checkin.xlsx")
    ...
    #check_in_df = fc.process_student_check_in(df, 423517)
    #check_in_df = fc.process_student_check_in(df, 423518)
    #check_in_df = fc.process_student_check_in(df, 423519)
    #check_in_df = fc.process_student_check_in(df, 423520)
    #check_in_df = fc.process_student_check_in(df, 423521)
    #check_in_df = fc.process_student_check_in(df, 423521)
    #print(df['barcode'].values)
    time_in = time.monotonic()
